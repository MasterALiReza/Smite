...
class BackhaulManager:
    ...
    async def start_server(self, tunnel_id: str, spec: dict) -> bool:
        ...
        try:
            proc = await start_async_process(cmd, str(self.config_dir), log_fh)
        except Exception:
            log_fh.close()
            raise
        ...
        if proc.returncode is not None:
            stderr = await read_log_tail(log_path)
            error_msg = f"Backhaul server failed to start (exit code {proc.returncode}). Log tail: {stderr}"
