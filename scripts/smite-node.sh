...
if [ ! -f "Dockerfile" ]; then
    ...
    TEMP_DIR=$(mktemp -d)
    git clone --depth 1 $GIT_BRANCH https://github.com/MasterALiReza/Smite.git "$TEMP_DIR" || {
...
if [ -f "/opt/smite/cli/smite-node.py" ]; then
    ...
    sudo curl -L https://raw.githubusercontent.com/MasterALiReza/Smite/${CLI_BRANCH}/cli/smite-node.py -o /usr/local/bin/smite-node
    sudo chmod +x /usr/local/bin/smite-node
fi
